...
<body>
    ...
    /// </summary>
    public async Task SendOrderStatusUpdateEmailAsync(OrderDto order, string customerEmail, CancellationToken cancellationToken = default)
    {
        try
        {
            var fromEmail = _configuration["EmailSettings:FromEmail"] ?? "noreply@eshop.com";
            ...
            {
                From = new MailAddress(fromEmail, fromName),
                Subject = $"Order Status Update - {order.OrderName}",
                Body = BuildOrderStatusUpdateEmailBody(order),
                IsBodyHtml = true
            };
        ...
        catch (Exception ex)
        {
            ...
            // Note: We don't queue for retry here to avoid spamming in case of frequent status changes.
            // You might want to implement a more sophisticated retry strategy for status updates.
        }
    }

    private string BuildOrderStatusUpdateEmailBody(OrderDto order)
    {
        var itemsHtml = string.Join("", order.OrderItems.Select(item =>
        {
            var productName = !string.IsNullOrEmpty(item.ProductName) ? item.ProductName : $"Product {item.ProductId}";
...
<head>
    ...
    <style>
        ...
        .content {{ padding: 20px; background-color: #f9f9f9; }}
        table {{ width: 100%; border-collapse: collapse; margin: 20px 0; background-color: white; }}
        th {{ background-color: #FFA500; color: white; padding: 12px; text-align: left; }}
        .total {{ font-weight: bold; font-size: 1.2em; text-align: right; padding: 20px 0; }}
        .footer {{ text-align: center; padding: 20px; color: #777; font-size: 0.9em; }}
    </style>
</head>
<body>
    <div class='container'>
        ...
        <div class='content'>
            ...
            <p>The status of your order <strong>{order.OrderName}</strong> has been updated to <strong>{order.OrderStatus}</strong>.</p>
    ...
    /// </summary>
    public async Task SendOrderCancelledEmailAsync(OrderDto order, string customerEmail, CancellationToken cancellationToken = default)
    {
        try
        {
            var fromEmail = _configuration["EmailSettings:FromEmail"] ?? "noreply@eshop.com";
            ...
            {
                From = new MailAddress(fromEmail, fromName),
                Subject = $"Order Cancellation - {order.OrderName}",
                Body = BuildOrderCancelledEmailBody(order),
                IsBodyHtml = true
            };
    ...
    private string BuildOrderCancelledEmailBody(OrderDto order)
    {
        var itemsHtml = string.Join("", order.OrderItems.Select(item =>
        {
            var productName = !string.IsNullOrEmpty(item.ProductName) ? item.ProductName : $"Product {item.ProductId}";
...
<head>
    ...
    <style>
        ...
        .content {{ padding: 20px; background-color: #f9f9f9; }}
        table {{ width: 100%; border-collapse: collapse; margin: 20px 0; background-color: white; }}
        th {{ background-color: #FF0000; color: white; padding: 12px; text-align: left; }}
        .total {{ font-weight: bold; font-size: 1.2em; text-align: right; padding: 20px 0; }}
        .footer {{ text-align: center; padding: 20px; color: #777; font-size: 0.9em; }}
    </style>
</head>
<body>
    <div class='container'>
        ...
        <div class='content'>
            ...
            <p>Your order <strong>{order.OrderName}</strong> has been cancelled.</p>
